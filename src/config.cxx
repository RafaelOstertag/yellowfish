...
bool wantHiresOnly(int argc, char** argv) {
    for (int i = 1; i < argc; i++) {
        if (std::strcmp(argv[i], "hiresonly") == 0) {
            return true;
        }
    }
...
void configure(int argc, char** argv, Config& config) {
    ...
    config.imageRetrievers =
        std::vector<std::shared_ptr<imageretriever::ImageRetriever>>{
            std::make_shared<imageretriever::ImageServer>(
                config.width, config.height, config.hiresOnly),
